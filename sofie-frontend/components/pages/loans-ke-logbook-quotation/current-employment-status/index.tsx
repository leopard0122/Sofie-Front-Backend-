...
const radios: RadioItemType[] = [
  { keyboard: "A", label: "Probation", value: "Probation" },
  { keyboard: "B", label: "Contract", value: "Contract" },
  { keyboard: "C", label: "Permanent", value: "Permanent" },
];

const CurrentEmploymentStatus = () => {
  const main = useSelector((state: { main: StateType }) => state.main);
  const selected = main.formData?.current_employment_status;
  ...
  const submitFunction = () => {
    dispatch(
      setFormDataInfo({
        formData: {
          current_employment_status: formData,
        },
      })
    );
  };
  ...
  return (
    <ContentWrapper
      title={`What is your current employment status with ${main.formData.where_current_employed}?*`}
      buttonLabel="OK"
      ButtonIcon={Check}
      nextPage="8"
