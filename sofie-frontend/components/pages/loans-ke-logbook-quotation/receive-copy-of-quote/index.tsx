...
const ReceiveCopyOfQuote = () => {
  const main = useSelector((state: { main: StateType }) => state.main);
  const selected = main.formData?.receive_copy_of_quote;
  const dispatch: any = useDispatch();
  const regExp = /^([\w\.\+]{1,})([^\W])(@)([\w]{1,})(\.[\w]{1,})+$/;
  const [nextPage, setNextPage] = useState<string>("29");
  ...
  const submitFunction = async () => {
    if (regExp.test(formData)) {
      await dispatch(
        setFormDataInfo({
          formData: {
            receive_copy_of_quote: formData,
          },
        })
      );
      dispatch(
...
export default ReceiveCopyOfQuote;
