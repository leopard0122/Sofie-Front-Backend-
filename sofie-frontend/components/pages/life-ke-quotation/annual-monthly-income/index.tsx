...
const AnnualMonthlyIncome = () => {
  const selected = useSelector((state: { main: StateType }) => state.main).formData?.annual_monthly_income;
  ...
  const submitFunction = () => {
    dispatch(
      setFormDataInfo({
        formData: {
          annual_monthly_income: formData?.value,
        },
      })
    );
  };
  ...
  return (
    <ContentWrapper
      title="What is your annual pre-tax income?*"
      description="Figures in USD"
      buttonLabel="OK"
      ButtonIcon={Check}
      nextPage="11"
...
export default AnnualMonthlyIncome;
